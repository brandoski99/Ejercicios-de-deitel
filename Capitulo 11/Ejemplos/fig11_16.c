...
actualiza los datos
ya escritos en el archivo, crea nuevos datos para colocarlos en el
archivo, y elimina
...
int main() {
  FILE *ptrCf;
  int eleccion;
  /* apuntador de archivo credito.dat */
  ...
  if ((ptrCf = fopen("credito.dat", "rb+")) == NULL) {
    ...
    while ((eleccion = intOpcion()) != 5) {
      ...
      case 4:
        ...
        break;
      /* si el usuario no selecciona una opción válida, despliega un
      mensaje */
      default:
...
void archivoTexto(FILE *ptrLee) {
  ...
  if ((ptrEscribe = fopen("cuentas.txt", "w")) == NULL) {
    ...
    fprintf(ptrEscribe, "%-6s%-16s%-11s%10s\n", "Cta", "Apellido", "Nombre",
            "Saldo");
    /* copia todos los registros del archivo de acceso aleatorio dentro del
    archivo de texto */
    while (!feof(ptrLee)) {
...
void actualizaRegistro(FILE *ptrF) {
  ...
  if (cliente.numCta == 0) {
    ...
    printf("%-6d%-16s%-11s%10.2f\n\n", cliente.numCta, cliente.apellido,
           cliente.nombre,
           cliente.saldo); /* pide al usuario el monto de la transacción */
    ...
    fseek(ptrF, (cuenta - 1) * sizeof(struct datosCliente), SEEK_SET);
    /* escribe el registro actualizado sobre el registro anterior en el
    archivo */
...
void eliminaRegistro(FILE *ptrF) {
  ...
  struct datosCliente clienteEnBlanco = {0, "", "", 0}; /* cliente en
blanco */
  ...
  if (cliente.numCta == 0) {
    ...
    /* mueve el apuntador de archivo hacia el registro correcto en el
    archivo */
    fseek(ptrF, (numCuenta - 1) * sizeof(struct datosCliente), SEEK_SET);
